    ...
    <header className='lg:rounded-20 border-stroke-soft-200 lg:shadow-complex relative z-400 flex h-16 w-full items-center justify-between gap-5 border-b bg-bg-white-0 px-4.5 py-4.5 lg:mx-auto lg:mt-6 lg:w-fit lg:min-w-160 lg:border-none lg:px-5 lg:py-4'>
      <div className='flex flex-1 items-center gap-4 lg:gap-3'>
        <div className='flex' ref={toggleRef}>
          <Button.Root
            ...
            onClick={toggleMenu}
          >
            <Button.Icon as={RiMenuLine} className='text-text-sub-600 size-5' />
          </Button.Root>
          <Button.Root
            variant='neutral'
            mode='ghost'
      ...
      <div className='flex items-center gap-3'>
        ...
        <div className='group/profile flex w-auto cursor-pointer items-center gap-1 bg-transparent p-0'>
          ...
          />
          <RiArrowDownSLine className='text-text-sub-600 group-hover:text-text-strong-950 size-5 transition-all duration-300 group-hover/profile:-rotate-180' />
        </div>
      </div>
      <div
        ref={panelRef}
        ...
        className={`lg:shadow-complex ease absolute top-16 left-0 h-[calc(100dvh-64px)] w-full overflow-auto transition-all duration-500 lg:top-20.5 lg:left-1/2 lg:h-auto lg:w-fit lg:-translate-x-1/2 lg:overflow-hidden lg:rounded-3xl ${isMenuOpen ? 'visible opacity-100' : 'invisible opacity-0'}`}
      >
