...
type FileData = {
  id: string;
  fileName: string;
  fileType: 'pdf' | 'doc' | 'docx' | 'pptx' | 'xlsx';
  fileSize: string;
  uploadedBy: {
    name: string;
    email: string;
...
const data: FileData[] = [
  ...
  },
  {
    ...
    uploadDate: 'July 23, 2023',
    lastUpdated: 'July 25, 2023',
  },
  {
    id: '6',
    fileName: 'policy-guidelines.doc',
    fileType: 'doc',
    fileSize: '1.2 MB',
    uploadedBy: {
      name: 'Laura Perez',
  ...
  },
  {
    ...
    uploadDate: 'July 27, 2023',
    lastUpdated: 'July 29, 2023',
  },
];
...
export default function FileTable() {
  ...
  const filteredData = React.useMemo(() => {
    ...
    if (filter === 'employee')
      return data.filter(
        (file) =>
          file.fileName.includes('employee') ||
          file.fileName.includes('training'),
      );
    if (filter === 'company')
      return data.filter(
        (file) =>
          file.fileName.includes('project') ||
          file.fileName.includes('meeting') ||
          file.fileName.includes('marketing') ||
          file.fileName.includes('policy') ||
          file.fileName.includes('budget'),
      );
    return data;
  }, [filter]);
  ...
  const columns: ColumnDef<FileData>[] = [
    ...
    },
    {
      ...
      header: ({ column }) => (
        <div className='flex items-center gap-0.5'>
          <span className='text-paragraph-sm text-text-sub-600'>File Name</span>
          <button
            type='button'
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className='flex items-center gap-3'>
          <Avatar.Root size='40' color={row.original.uploadedBy.color}>
            <Avatar.Image src={row.original.uploadedBy.avatar} />
          </Avatar.Root>
          <div className='flex flex-col gap-0.5'>
            <span className='text-paragraph-sm text-text-strong-950'>
        ...
        <div className='w-full sm:w-[320px]'>
          <SegmentedControl.Root defaultValue='all' onValueChange={setFilter}>
            <SegmentedControl.List>
              ...
              <SegmentedControl.Trigger value='employee'>
                Employee
              </SegmentedControl.Trigger>
